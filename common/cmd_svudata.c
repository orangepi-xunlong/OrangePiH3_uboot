...
char USER_DATA_NAME[12][NAME_SIZE] = {{'\0'}};		//用户的环境变量（从env.fex获取）
...
************************************************************************************************************
*/
int wifi_mac_updated = -1;//indicate read wifi_mac from efuse successfully
int update_user_data(void)
{
	...
	wifi_mac_updated = update_mac_from_efuse();
	...
	{	
		...
		{
			if(!strncmp(USER_DATA_NAME[i],"wifi_mac",8) && !wifi_mac_updated)
				continue;
		...
		if (user_data_head->count > 0) {
			for (i = 0; i < USER_DATA_NUM; i++) {
				...
				for (j = 0; j < user_data_head->count && j < USER_DATA_PARAMETER_MAX_COUNT; j++) {
					...
					{
						if(!strcmp(USER_DATA_NAME[i],"wifi_mac") && !wifi_mac_updated)
							continue;
