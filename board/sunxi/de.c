...
int get_hdmi_vendor_id(unsigned char * edid_buf);
...
int reset_hdmi_mode(int sel, int mode)
{
	...
	memset((void *)edid_buf, 0, sizeof(edid_buf));
	get_hdmi_edid(sel, edid_buf, 1024);

	if(-2 == edid_checksum((char const *)edid_buf)){
	    hdmi_mode_check = 0;
	} else if(script_parser_fetch("boot_disp", "hdmi_mode_check", &hdmi_mode_check, 1) < 0){
		hdmi_mode_check = 1;
	}
	if(2 == hdmi_mode_check){
		// if vendor id change , check mode: hdmi_mode_check = 1;
		hdmi_vendor_id = get_hdmi_vendor_id(edid_buf);
	...
	if(1 == hdmi_mode_check){  //MY_HDMI_SUPPORT_MODE: check if support the output_mode by television, return 0 is not support
		arg[0] = sel;
		arg[1] = mode;
		if(1 == disp_ioctl(NULL, MY_HDMI_SUPPORT_MODE, (void*)arg)) {
			return mode;
		}
