...
/*--------------------------------------------------------------------*/
/*--------------------------------------------------------------------*/
//some wifi modules do not have space to save user-defined wifi-mac,so save it in soc efuse
//when write,must transfrom 11:22:33:44:55:66(17bytes) to 112233445566(6bytes) to save soc efuse space
//when read ,must transfrom 112233445566(6bytes) to 11:22:33:44:55:66(17bytes)
...
char *sunxi_mac_char_to_int(char *mac_char)
{
	char *result = malloc(7);
	int i=0,tmp0 = 0,tmp1 = 0;
	if(mac_char == NULL)
		return -1;
	memset(result,0x00,7);
	for(i = 0; i<6; i++)
	{
		...
		if(mac_char[3*i+2] != ':' && i <= 4 )
		{
			printf("wifi mac char format is wrong\n");
			return -1;
		}
		printf("result = %02x\n",result[i]);
	}
	result[6] = 0xAA; //a flag indicates that have burned wifi mac
	return result;
}
//transfrom 112233445566(6bytes) to 11:22:33:44:55:66(17bytes)
char* sunxi_mac_int_to_char(char *mac_int)
{
    char *result = malloc(18);
	int i;
	if(mac_int == NULL)
		return -1;
	memset(result,0x00,18);
	for(i=0;i<6;i++)
	{
...
{
	int ret = -1,i,validate = 0;
	char *wifi_mac_int= malloc(7);
	char *wifi_mac_char = NULL;
	if(wifi_mac_int == NULL)
	{
		printf("malloc for wifi mac failed\n");
		return -1;
	}
	memset(wifi_mac_int,0x00,7);
	ret = sunxi_efuse_read("wifi_mac_efuse",wifi_mac_int);
	if(ret == -1)
	{
    ...
    if(wifi_mac_int[6] != 0xAA)
    {
		printf("have not burned wifi_mac to efuse\n");
		free(wifi_mac_int);
		return -1;
	}		
	wifi_mac_char = sunxi_mac_int_to_char(wifi_mac_int);
	setenv("wifi_mac", wifi_mac_char);
	return 0;
}

//we must check whether key exist or not before burning
int check_mac_from_efuse(void)
{
	int ret = -1,i;
	char *wifi_mac_int= malloc(7);
	char *wifi_mac_char = NULL;
	...
	if(wifi_mac_int[6] == 0xAA)//have burned wifi_mac
		return  1;
	
    for(i=0;i<7;i++)//efuse has dirty data that we dont want
	{
		if(wifi_mac_int[i] != 0x00)
		{
			free(wifi_mac_int);
			return -1;
		}
	...
	free(wifi_mac_int);
	return 0;//can burn wifi_mac to efuse
}