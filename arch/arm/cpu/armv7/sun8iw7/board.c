...
int sunxi_probe_securemode(void)
{
	...
	{
		//secure boot will set secureos_exist, the default val is -1.
		if(uboot_spare_head.boot_data.secureos_exist == 0)
		{
				printf("SUNXI_SECURE_MODE_NO_SECUREOS \n");
				gd->securemode = SUNXI_SECURE_MODE;
				gd->bootfile_mode = SUNXI_BOOT_FILE_TOC;
		}
		else if(uboot_spare_head.boot_data.secureos_exist == 1)
		{
				printf("SUNXI_SECURE_MODE_WITH_SECUREOS \n");
	...
	else
	{
		...
		mode = 	readl(CONFIG_SYS_SRAMA2_BASE);
		if(mode!=0xffff)
		{
			printf("usb burn:SUNXI_SECURE_MODE_NO_SECUREOS\n");
			gd->bootfile_mode = SUNXI_BOOT_FILE_TOC;
