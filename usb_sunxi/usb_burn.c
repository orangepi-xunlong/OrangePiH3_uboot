...
extern char *sunxi_mac_char_to_int(char* mac_char);
...
int __sunxi_burn_key(u8 *buff, uint buff_len)
{
	...
	for(;key_count>0;key_count--, key_list++)
	{
		...
		if(!strncmp(key_list->name,"wifi_mac_efuse",14))
			key_list->type = 0;
#ifdef CONFIG_SUNXI_SECURE_SYSTEM
		if(!key_list->type) //burn keydata to soc efuse
		{
			//step 1 : construct the efuse_key_info (key-independent code)
		    ...
		    //step 2 : process some special keys (key-dependent code)
			...
			else if(!strcmp(efuse_key_info.name, "wifi_mac_efuse"))
			{
				int i = 0;
				char *wifi_mac_int = sunxi_mac_char_to_int((char *)key_list->key_data);
	
				for(i=0;i<7;i++)
					printf("%02x",wifi_mac_int[i]);
				
				if(check_mac_from_efuse())
				{
					printf("nv2 have data in low 7 bytes,abort burn key\n");
					return -1;
				}
				efuse_key_info.len = 7;
				efuse_key_info.key_data = (u8 *)wifi_mac_int;
			}
            //step 3 : burn keydata to efuse (key-independent code)
			if(sunxi_efuse_write(&efuse_key_info))
			{
				printf("burn key:%s to efuse failed\n",key_list->name);
				return -1;
			}
		}
		else
...
int __sunxi_read_key_by_name(void *buffer, uint buff_len, int *read_len)
{
	...
	*read_len = sizeof(sunxi_usb_burn_key_info_t);
	...
	if(!strncmp(key_info->name,"wifi_mac_efuse",14))
		key_info->type = 0;
#ifdef CONFIG_SUNXI_SECURE_SYSTEM
	if(!key_info->type)
	{
		//read from efuse
		if(sunxi_efuse_read(key_info->name, (void *)data_buff))
		{
		...
		if(!strncmp(key_info->name,"wifi_mac_efuse",14) && check_mac_from_efuse() == 1)// special process for wifi_mac
		{
			int i;
			printf("read wifi_mac from efuse\n");
			key_info->len = 17;
            char *wifi_mac_char = sunxi_mac_int_to_char(data_buff);
			for(i=0;i<6;i++)
				printf("%02x",data_buff[i]);
			printf("\n");
			memcpy((void *)(key_info->key_data), (void *)(wifi_mac_char),key_info->len);
			*read_len += 17;
		}
		else if(!strncmp(key_info->name,"rotpk",5))
		{
			memcpy((void *)(key_info->key_data), (void *)(data_buff), 32);
			*read_len += 32;
		}
		else //normal procese
		{
