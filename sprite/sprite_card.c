...
int sunxi_sprite_deal_recorvery_boot(int production_media)
{
    ...
    if(!production_media)//if media is nand
		sunxi_flash_exit(1);//important for nand logic interface
...
int card_download_boot0(uint length, void *buffer)
{
	...
	printf("burn boot0/toc0 at emmc %d sector ok\n",BOOT0_SDMMC_START_ADDR);
	ret = sunxi_sprite_phywrite(BOOT0_BACKUP_SDMMC_START_ADDR, length/512, buffer);
	if(!ret)
	{
		return -1;
	}
    printf("burn boot0/toc0 backup at emmc %d sector ok\n",BOOT0_BACKUP_SDMMC_START_ADDR);
	return 0;
}
/*
************************************************************************************************************
